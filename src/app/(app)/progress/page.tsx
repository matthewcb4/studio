...
export default function ProgressPage() {
  ...
  const { data: workoutLogs, isLoading: isLoadingWorkoutLogs } = useCollection<WorkoutLog>(workoutLogsQuery);
  
  const progressLogsQuery = useMemoFirebase(() => {
    if (!user) return null;
    return query(collection(firestore, `users/${user.uid}/progressLogs`), orderBy("date", "asc"));
  }, [firestore, user]);
  const { data: progressLogs, isLoading: isLoadingProgressLogs } = useCollection<ProgressLog>(progressLogsQuery);
  ...
  const onWeightSubmit = async (values: z.infer<typeof weightLogSchema>) => {
    ...
    const progressLogCollection = collection(firestore, `users/${user.uid}/progressLogs`);
    try {
    ...
    <div className="flex flex-col gap-8">
      ...
      <div className="grid gap-6 md:grid-cols-1 lg:grid-cols-2">
        <Card>
            <CardHeader>
                ...
                <CardDescription>Enter your current weight for today.</CardDescription>
            </CardHeader>
            <Form {...weightForm}>
                <form onSubmit={weightForm.handleSubmit(onWeightSubmit)}>
                    <CardContent>
                        <FormField
                            ...
                            render={({ field }) => (
                                ...
                                <FormControl>
                                    <Input type="number" placeholder="e.g., 185.5" {...field} />
                                </FormControl>
                                <FormMessage />
                                </FormItem>
                            )}
                    ...
                    <CardFooter>
                        <Button type="submit" disabled={isSubmittingWeight}>
                            {isSubmittingWeight && <Loader2 className="mr-2 h-4 w-4 animate-spin" />}
                            Log Weight
                        </Button>
                    </CardFooter>
                </form>
                ...
                {!isLoading && weightChartData.length > 0 ? (
                    <ChartContainer config={weightChartConfig} className="h-[200px] w-full">
                        <ResponsiveContainer width="100%" height="100%">
                            <LineChart data={weightChartData as any} margin={{ top: 5, right: 20, left: -10, bottom: 5 }}>
                                <CartesianGrid strokeDasharray="3 3" vertical={false} />
                                <XAxis dataKey="date" tickFormatter={(value) => format(new Date(value), 'MMM d')} tickLine={false} axisLine={false} />
                                <YAxis dataKey="weight" domain={['dataMin - 5', 'dataMax + 5']} axisLine={false} tickLine={false} />
                                <ChartTooltip cursor={false} content={<ChartTooltipContent indicator="dot" />} />
                                <Line type="monotone" dataKey="weight" stroke="hsl(var(--accent))" strokeWidth={2} dot={{ r: 4, fill: "hsl(var(--accent))" }} />
                            </LineChart>
                        </ResponsiveContainer>
                    </ChartContainer>
                ) : (
                    !isLoading && <p className="text-sm text-muted-foreground">No weight logged yet.</p>
                )}
            </CardContent>
        </Card>
      </div>
      ...
      <Card>
        <CardHeader>
            ...
            <CardDescription>Select an exercise to see your lift progression.</CardDescription>
        </CardHeader>
        <CardContent className="space-y-4">
            ...
            {isLoading && <div className="flex flex-col items-center justify-center p-12"><CardHeader className="text-center"><CardTitle>Loading Progress Data...</CardTitle></CardHeader></div>}
            {!isLoading && exerciseChartData.length > 0 ? (
                <ChartContainer config={maxWeightChartConfig} className="h-[300px] w-full">
                    <ResponsiveContainer width="100%" height="100%">
                        <LineChart data={exerciseChartData as any} margin={{ top: 5, right: 20, left: -10, bottom: 5 }}>
                        <CartesianGrid strokeDasharray="3 3" vertical={false} />
                        <XAxis
                            dataKey="date"
            ...
            !isLoading && <div className="flex flex-col items-center justify-center p-12 border-2 border-dashed rounded-lg">
